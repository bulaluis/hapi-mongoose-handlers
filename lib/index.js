...
var internals = {
    schema: Joi.object({
        onCreate: Joi.string().valid('object', 'no-content'),
        onUpdate: Joi.string().valid('object', 'no-content'),
        onRemove: Joi.string().valid('object', 'no-content'),
        meta: Joi.object({
            totalPagesKey: Joi.string(),
            totalDocsKey: Joi.string(),
        ...
        where: Joi.boolean(),        // At your own risk
    }),
    defaults: {
        onCreate: 'no-content',
        onUpdate: 'no-content',
...
exports.register = function (server, options, next) {
    ...
    // Just and necessary
    server.dependency('hapi-mongoose-request');
...
internals.getId = function (request) {
...
internals.handlers.find = function (route, options) {
    ...
    return function (request, reply) {
        ...
        var id = internals.getId(request);
        var query = id ? Model.findById(id) : Model.find(request.pre.conditions || {});
        ...
        if (page) {
            if (!limit) {
                limit = settings.meta.defaultLimit;
            }
        ...
        Async.parallel({
            ...
            Async.waterfall([
                ...
                var meta = Lodash.zipObject([settings.meta.totalPagesKey, settings.meta.totalDocsKey], [Math.ceil(data.count / limit) || 1, data.count]);
                return reply(err, Lodash.zipObject([Model.modelName, 'meta'], [docs, meta]));
            });
        });
    };
};
...
internals.handlers.find.defaults = {
    validate: {
        ...
        }).with('deepPopulate', 'populate').unknown(true)          // To deepPopulate is required populate
    }
};
